...
All metrics can be used with the [Evalite](https://evalite.dev) evaluation framework using `toEvaliteScorer`. This allows you to leverage Evalite's experiment tracking, comparison tools, and web UI while using the production-ready metrics from this package.
...
See the [Evalite Integration](/docs/metrics/evalite) guide for complete documentation.
