...
<TypeTable
  type={{
    graph: {
      description: 'The knowledge graph to generate test data from',
      type: 'KnowledgeGraph',
      required: true,
    },
    synthesizers: {
      description:
        'Array of [synthesizer, weight] tuples. Weights determine the distribution of samples',
      type: '[Synthesizer, number][]',
      required: true,
    },
    personas: {
      description: 'The personas to generate test data for',
      type: 'Persona[]',
      required: true,
    },
    count: {
      description: 'The number of test samples to generate',
      type: 'number',
      required: true,
    },
    config: {
      description: 'Configuration options for synthesis',
      type: 'object',
    },
    'config.concurrency': {
      description: 'Number of samples to generate in parallel',
      type: 'number',
      default: '5',
    },
    'config.generateGroundTruth': {
      description: 'Whether to generate reference answers for each sample',
      type: 'boolean',
      default: 'true',
    },
  }}
